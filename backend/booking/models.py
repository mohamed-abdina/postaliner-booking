...
def generate_reference():
    return "PL-" + "".join(secrets.choice(string.ascii_uppercase + string.digits) for _ in range(7))
...
class Booking(models.Model):
    ...
    class Meta:
        ordering = ["-created_at"]
        indexes = [
            models.Index(fields=["schedule", "travel_date"], name="idx_booking_schedule_date"),
        ]
...
class SeatHold(models.Model):
    ...
    class Meta:
        ordering = ["-created_at"]
        indexes = [
            models.Index(fields=["schedule", "travel_date"], name="idx_seathold_schedule_date"),
        ]
